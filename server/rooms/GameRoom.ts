...
import { GameState, Player, Collectible, GridCell, PlayerColor, CollectibleType, CollectibleColor, CollectibleOrientation, Enemy, EnemyPersonality, Mine, MineType } from "../schema/GameState";
...
export class GameRoom extends Room<GameState> {
  ...
  onCreate(options: any) {
    ...
    // TEMP MINE TEST TOOL: same as dev stage-up, but available to the hidden local test console.
  ...
  private async initializeGame() {
    ...
    // Generate initial collectibles. Stage 1 intentionally starts without mines.
    this.generateInitialCollectibles();
    this.dropMinesForCurrentStage();
    this.calculateScores();
  ...
  private isPositionOccupied(x: number, y: number): boolean {
    ...
    // Check if any prototype mine is at this position
  ...
  private isInVisibleMineBounds(x: number, y: number): boolean {
    const { minX, maxX, minY, maxY } = this.getVisibleMineBounds();
    return x >= minX && x <= maxX && y >= minY && y <= maxY;
  }

  private addBlastCell(cells: Map<string, { x: number; y: number }>, x: number, y: number) {
    if (!this.isInVisibleMineBounds(x, y)) return;
    cells.set(`${x},${y}`, { x, y });
  }

  private getExplosionCellsForMine(mine: Mine): Array<{ x: number; y: number }> {
  ...
  private dropMinesForCurrentStage() {
    // Colored mine rule: do not clear old unexploded mines.
    // Each new stage adds a fresh balanced batch inside the visible board.
    const minesToDrop = this.getMineDropCountForStage(this.state.stage);
    const minesPerColor = this.getMinesPerColorForStage(this.state.stage);

    if (minesToDrop <= 0) {
      console.log(`Stage ${this.state.stage}: no mines dropped`);
      return;
    }
    ...
    const placeMineAt = (x: number, y: number, color: PlayerColor, type: MineType): boolean => {
      ...
      const mine = new Mine();
      mine.id = `${color}-${type}-mine-stage-${this.state.stage}-${Date.now()}-${minesPlaced}`;
      mine.x = x;
      mine.y = y;
      mine.color = color;
      mine.type = type;
    ...
    for (const mineColor of colors) {
      for (let colorDropIndex = 0; colorDropIndex < minesPerColor; colorDropIndex++) {
        const mineType = this.getMineTypeForStage(this.state.stage);
        let placedThisMine = false;

        let attempts = 0;
        const maxAttempts = 150;
        while (!placedThisMine && attempts < maxAttempts) {
          attempts++;
  ...
  private detonateMine(
    ...
    const blastCellKeys = new Set<string>();

    for (const cell of blastCells) {
      const key = `${cell.x},${cell.y}`;
      blastCellKeys.add(key);
      this.state.gridColors.delete(key);
    }

    this.broadcast("mineTriggered", {
      x: detonatedMine.x,
      y: detonatedMine.y,
      color: detonatedMine.color,
      mineType,
      source,
      chainDepth,
  ...
  private advanceToStage(newStage: number) {
    ...
    if (this.levelSpec) {
      ...
      this.dropMinesForCurrentStage();
      this.calculateScores();
      console.log(`Stage ${newStage}: Visible area expanded from ${oldGridWidth}x${oldGridHeight} to ${this.state.gridWidth}x${this.state.gridHeight} (using level spec), dropped new mines`);
      return;
    }
    ...
    console.log(`Stage ${newStage}: Visible area expanded from ${oldGridWidth}x${oldGridHeight} to ${this.state.gridWidth}x${this.state.gridHeight}, added ${totalCollectiblesAdded} new collectibles, dropped new mines`);
  }
