...
import { Mine, MineExplosion } from "@/components/Mine";
...
export const GameScreen = ({
  ...
  const mineViewerColor = useMemo((): PlayerColor | null => {
    if (isSpectator) return null;
    const playerArray = Array.from(players.values());
    return isSoloMode
      ? playerArray[activePlayerIndex]?.color ?? null
      : myColor;
  ...
  useEffect(() => {
    ...
    const playMineExplosionSound = () => {
      try {
        ...
        const audioWindow = window as Window & { __polarMineAudioCtx?: AudioContext };
        const ctx = audioWindow.__polarMineAudioCtx ?? new AudioCtx();
        audioWindow.__polarMineAudioCtx = ctx;
        ...
        thump.frequency.exponentialRampToValueAtTime(34, now + 0.32);
        thumpGain.gain.setValueAtTime(0.0001, now);
        thumpGain.gain.exponentialRampToValueAtTime(0.95, now + 0.012);
        ...
        // Short crack/noise burst so it reads more like an explosion and less like a UI beep.
        ...
        noiseFilter.frequency.exponentialRampToValueAtTime(420, now + 0.24);
        noiseGain.gain.setValueAtTime(0.0001, now);
        noiseGain.gain.exponentialRampToValueAtTime(0.72, now + 0.006);
        ...
        // Browser audio can fail before user interaction; do not break gameplay.
      }
    };

    const handleMineTriggered = (message?: {
      x?: number;
      y?: number;
      ...
      if (typeof message?.x === "number" && typeof message?.y === "number") {
        const explosionId = `${Date.now()}-${message.x}-${message.y}-${Math.random()}`;
        setMineExplosions((current) => [
          ...current,
          {
            id: explosionId,
            x: message.x!,
            y: message.y!,
            type: message.mineType ?? "square",
          },
        ]);
        window.setTimeout(() => {
          setMineExplosions((current) => current.filter((explosion) => explosion.id !== explosionId));
        }, 720);
      }
    ...
    <div className="isolate w-full h-screen relative overflow-hidden bg-canvas">
      ...
      >
        ...
        <ParticleFloor key={`floor-${gridWidth}-${gridHeight}`} gridWidth={gridWidth} gridHeight={gridHeight} spacing={SPACING} nodeStates={nodeStates} rippleTrigger={rippleTrigger} />
            ...
            {/* Quick blast animation only; no lingering after-effect. */}
            {mineExplosions.map((explosion) => {
              const pos = getVisualPos(explosion.x, explosion.y, -1.35);
